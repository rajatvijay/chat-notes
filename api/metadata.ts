...
import {
  callOpenAI,
  enhanceReadingContent,
  buildCategoryPrompt,
} from './shared/openai-utils.js'
import {
  updateNoteMetadata,
  getSupabaseClient,
} from './shared/supabase-utils.js'
  ...
  return withAuth(req, res, async (req, res) => {
    ...
    if (body.action === 'extract') {
      ...
      if (
        !extractBody.note_id ||
        !extractBody.content ||
        !extractBody.category
      ) {
        return res
          .status(400)
          .json({ error: 'Missing note_id, content, or category' })
      }

      // Skip metadata extraction for journal and misc
      if (
        extractBody.category === 'journal' ||
        extractBody.category === 'misc'
      ) {
        return res.status(200).json({ metadata: {} })
      }

      const schema =
        metadataSchemas[extractBody.category as keyof typeof metadataSchemas]
      if (!schema) {
      ...
      try {
        metadata = JSON.parse(openaiData.choices[0].message.content) as Record<
          string,
          unknown
        >
      } catch (e) {
      ...
      if (
        !meetingBody.note_ids ||
        !Array.isArray(meetingBody.note_ids) ||
        meetingBody.note_ids.length === 0
      ) {
        return res
          .status(400)
          .json({ error: 'Missing or invalid note_ids array' })
      }
      ...
      if (
        ...
        typeof updateMetadataBody.metadata !== 'object'
      ) {
        return res.status(400).json({ error: 'Missing or invalid metadata' })
      }
